...
export const MealSummary = ({meal, calories, p, f, c} : {meal: Meal, calories: number, p: number, f: number, c: number}) => (
    <AnimatePresence mode="wait">
        <Block key={meal} initial="init" animate="in" exit="out" className="light-block w-full px-6 md:px-12 py-6 flex flex-col justify-between min-h-64">
            <motion.div className="text-2xl" variants={variants} transition={{duration: 0.5}}>
                Ваш {meals[meal]}:
            </motion.div>
